...
export default function ListView() {
  ...
  return (
    <div className="bg-white flex-1 rounded-xl p-5">
      <h2 className="text-xl font-bold mb-4">Brand List</h2>
      
      {brands.length === 0 ? (
        <div className="text-center py-8 text-gray-500">
          No brands found. Create a new brand to get started.
        </div>
      ) : (
        <>
              ...
              <div key={brand.id} className="border rounded-lg p-4 shadow-sm">
                <div className="flex items-center gap-4">
                  <div className="flex-shrink-0">
                    {brand.image && typeof brand.image === 'string' && isValidUrl(brand.image) ? (
                      <img
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="font-medium text-gray-900 truncate">{brand.name || "Unnamed"}</h3>
                    <p className="text-sm text-gray-500 truncate">{brand.slug || "No slug"}</p>
                  </div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {brands.map((brand) => (
                  <tr key={brand.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-medium text-gray-900">{brand.name || "Unnamed"}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-500">{brand.slug || "No slug"}</div>
                    </td>
